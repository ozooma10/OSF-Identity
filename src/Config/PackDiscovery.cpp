...
namespace Config
{
    namespace 
    {
        ManifestResult BuildImplicitPackage(const std::filesystem::path& a_packageDirectory, const bool a_requirePresetFiles)
        {
            ...
            Detail::DiscoverConventionAssignments(manifest, result, a_requirePresetFiles);
    ...
    namespace Detail
    {
        void DiscoverConventionAssignments(PackageManifest& a_manifest, ManifestResult& a_result, const bool a_requirePresetFiles)
        {
            ...
            if (!std::filesystem::is_directory(packageRoot, ec) || ec) {
                if (a_requirePresetFiles) {
                    AddIssue(a_result, packageRoot, 0, "package_root_missing", "convention pack directory is missing");
                }
                return;
            }

            std::vector<std::filesystem::path> pluginDirectories;
            std::filesystem::directory_iterator rootIt{packageRoot, std::filesystem::directory_options::skip_permission_denied, ec };
            ...
            for (; !ec && rootIt != end; rootIt.increment(ec)) {
                if (rootIt->is_directory(ec) && !ec) {
                    ...
                    if (extension == ".npc") {
                        AddIssue(a_result, rootIt->path(), 0, "invalid_convention_layout", "preset and metadata files must be inside <OwningPlugin>/");
                    }
                } else if (!ec) {
                    AddIssue(a_result, rootIt->path(), 0, "invalid_convention_layout", "unsupported entry at the pack root");
                }
            }
            if (ec) {
            ...
            std::ranges::sort(pluginDirectories, [](const auto& a_left, const auto& a_right) {
                return Util::FoldASCII(a_left.filename().string()) <
                    Util::FoldASCII(a_right.filename().string());
            });
            ...
            for (const auto& pluginDirectory : pluginDirectories) {
                ...
                for (const auto& file : files) {
                    const auto folded = Util::FoldASCII(file.filename().string());
                ...
                for (const auto& file : files) {
                    ...
                    if (!MergeRequirements(a_manifest.requirements, presetRequirements, assignment.requirements, plugin)) {
            ...
            for (auto& candidate : candidates) {
                ...
                if (targetCounts[key] != 1) {
                    AddIssue(a_result, candidate.sourcePath, 0, "duplicate_target", "pack contains more than one convention preset for target " + key);
                    continue;
                }
    ...
    ManifestResult LoadPackageDirectory( const std::filesystem::path& a_packageDirectory,  const bool a_requirePresetFiles)
    {
        return BuildImplicitPackage(a_packageDirectory, a_requirePresetFiles);
    }

    DiscoveryResult DiscoverPacks(const std::filesystem::path& a_packsRoot, const bool a_requirePresetFiles)
    {
        ...
        std::ranges::sort(packageDirectories, [](const auto& a_left, const auto& a_right) {
            return  Util::FoldASCII(a_left.generic_string()) < Util::FoldASCII(a_right.generic_string());
        });

        for (const auto& packageDirectory : packageDirectories) {
            auto parsed = LoadPackageDirectory(packageDirectory, a_requirePresetFiles);
            result.issues.insert(result.issues.end(), std::make_move_iterator(parsed.issues.begin()), std::make_move_iterator(parsed.issues.end()));
            if (parsed.manifest) {
        ...
        for (const auto& package : result.packages) {
            ++counts[Util::FoldASCII(package.packageID)];
        }
        std::erase_if(result.packages, [&](const auto& package) {
            if (counts[Util::FoldASCII(package.packageID)] == 1) {
                return false;
            }
            result.issues.push_back({ package.DiagnosticPath(), 0, "duplicate_package_id", "every pack whose folder name collides case-insensitively with '" + package.packageID + "' was rejected" });
            return true;
        });
        return result;
    }
