...
namespace Config
{
    ...
    [[nodiscard]] ManifestResult ParsePackageManifest(std::string_view a_json, const std::filesystem::path& a_manifestPath, bool a_requirePresetFiles);
...
namespace Config::Detail
{
    ...
    [[nodiscard]] PresetMetadataResult LoadPresetMetadata( const std::filesystem::path& a_path, const std::filesystem::path& a_packageRoot);

    // Defined in PackageDiscovery.cpp; consumed by ParsePackageManifest when a manifest declares no explicit assignments.
    void DiscoverConventionAssignments(PackageManifest& a_manifest, ManifestResult& a_result, bool a_requirePresetFiles);
}
