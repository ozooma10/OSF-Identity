...
- `invalid_convention_layout` - an `.npc` is at the pack root, a directory is
...
- `invalid_preset` - the `.npc` file is empty, too large, inaccessible, or
...
The pack folders and `.npc` files must be loose files.
...
- `preset issue` - the file is malformed or does not satisfy the supported `.npc` export contract. Use an actual Creation Kit or compatible CharGenMenu/SFEE export; do not hand-edit it.
